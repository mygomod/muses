...
import (
	...
	"github.com/goecology/muses/pkg/utils"
)
	...
	for _, v := range self.files {
		for _, file := range v {
			if buildAllFiles || utils.InSlice(file, files) {
				...
				if err != nil {
					logger.DefaultLogger().Error("parse template file: " + file + ", err:" + err.Error())
		...
		if len(m) == 2 {
			...
			for _, otherFile := range others {
				if otherFile == m[1] {
					...
					if err != nil {
						logger.DefaultLogger().Error("template parse file err:" + err.Error())
			...
			for _, otherFile := range others {
				...
				if err != nil {
					f.Close()
					logger.DefaultLogger().Error("template file parse error, not success open file:" + err.Error())
					continue
				}
				data, err = ioutil.ReadAll(f)
				f.Close()
